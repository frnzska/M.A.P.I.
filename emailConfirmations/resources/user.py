...
class UserLogin(Resource):
    ...
    def post(cls):
        user_json = request.get_json()
        user_data = user_schema.load(user_json, partial=('email', )) #basically means ignore in this load the email if not present, needs to be explicit since nullable=true
...
class UserConfirmed(Resource):
    ...
    def get(cls, user_id: int):
        ...
        user.activated = True
        user.save_to_db()
        return make_response(render_template('confirmation_page.html', email=user.email))
